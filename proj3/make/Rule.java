...
class Rule {
    ...
    /** Add the target of DEPENDENT to my dependencies. Fixed. */
    ...
    void addCommands(List<String> commands) {
        if (_commands.isEmpty() || commands.isEmpty()) {
            ...
            throw new
                    IllegalStateException("Two non-empty command sets error.");
        }
    }
    ...
    private void checkFinishedDependencies() {
        for (int dependent: _depends.successors(_vertex)) {
            Rule rule = _depends.getLabel(dependent);
            if (rule.isUnfinished()) {
                error("Dependencies are not built error.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int dependent: _depends.successors(_vertex)) {
            Rule rule = _depends.getLabel(dependent);
            if (_time < rule.getTime() || rule.getTime() == null) {
                return true;
            }
        }
        return false;
