...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        for (int i = 1; i < segment.size(); i += 1) {
            ...
            Direction roadDir = road.direction();
            double roadLength = road.length();
            if (i < segment.size() - 1) {
                for (int j = i; j < segment.size() - 1; j += 1) {
                    Road adjRoad = _map.getLabel(segment.get(j),
                            segment.get(j + 1));
                    String adjRoadName = adjRoad.toString();
                    Direction adjRoadDir = adjRoad.direction();
                    double adjRoadLength = adjRoad.length();
                    if (adjRoadName.equals(roadName)
                            && adjRoadDir.equals(roadDir)) {
                        roadLength += adjRoadLength;
                        i = j + 1;
                    } else {
                        break;
                    }
                }
                String result = Integer.toString(seq) + "." + " Take "
                        + roadName + " " + roadDir.fullName() + " for "
                        + String.format("%.1f", roadLength) + " miles";
                if (i == segment.size() - 1) {
                    String to = _map.getLabel(segment.get(i)).toString();
                    result += " to " + to + ".";
                } else {
                    result += ".";
                }
                ...
                String result = Integer.toString(seq) + "." + " Take "
                        + roadName + " " + roadDir.fullName() + " for "
                        + String.format("%.1f", roadLength) + " miles to "
                        + to + ".";
                System.out.println(result);
                seq += 1;
            }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road fromTo = new Road(name, dir, length);
        Road toFrom = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, fromTo);
        _map.add(v1, v0, toFrom);
    }
